...
public class UseInputPartialModel implements IUseInputPartialModel {

	private EMFModel partialModel;
	// private IMetamodelRewrite rewrite;

	public UseInputPartialModel(@NonNull EMFModel partialModel) {
		this.partialModel = partialModel;
	}

	@Override
	public void makeUseObjects(@NonNull Session fSession) {
		UseSystemApi sysApi = UseSystemApi.create(fSession);
		
		// Pass 1: Transform all objects into USE objects
		TreeIterator<EObject> it = partialModel.getAllContents();
		while ( it.hasNext() ) {
			EObject obj = it.next();
			convert(obj, sysApi);
		}
		...
		/*
		it = partialModel.getAllContents();
		while ( it.hasNext() ) {
			EObject obj = it.next();
	...
	private void convert(EObject obj, UseSystemApi sysApi) {
		...
		String id = c.getName() + "_" + objCounter++;
		ecoreToUseId.put(obj, id);
		
		try {
			sysApi.createObject(c.getName(), id);
			for (EAttribute att : c.getEAllAttributes()) {
				if ( ! obj.eIsSet(att) )
					continue;
				
				// TODO: This needs to be supported specially in getValue
				if (att.isMany())
					continue;
